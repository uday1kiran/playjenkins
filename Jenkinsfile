          ...
          script {
            ...
            withCredentials([usernamePassword(credentialsId: 'testimage', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
              ...
              def shortCommitId = "${env.GIT_COMMIT}".take(7)
               sh 'docker tag uday1kiran/testimage:${env.BRANCH_NAME} uday1kiran/testimage:${env.BRANCH_NAME}-${shortCommitId}'
               sh 'docker push uday1kiran/testimage:${env.BRANCH_NAME}-${shortCommitId}'               
              }
